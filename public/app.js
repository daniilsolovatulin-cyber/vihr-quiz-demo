...
function showToast(msg, icon) {
  if (icon === ICONS.cross || icon === ICONS.wifi) Sound.error();
  ...
  return `
    <div class="shell-header">
      ...
      <div class="header-actions">
        ...
        <button class="icon-btn" id="btnSettings" title="Настройки">${ICONS.settings}</button>
        ${state.user
...
function bindProfile() {
  ...
  if (testBtn) testBtn.onclick = () => Sound.victory();
...
function finishQuiz() {
  ...
  if (total && state.score / total >= 0.9) { Sound.victory(); celebrate(); }
}
...
async function loadTodCard(type) {
  ...
  try {
    ...
    if (state.view !== 'tod') return;
    showToast('Не удалось загрузить карточку. Показываю запасную.', ICONS.wifi);
...
function connectRoomSocket(code, isReconnect) {
  ...
  ws.onclose = () => {
    ...
    if (roomReconnectAttempt === 0) Sound.error();
...
function playLeaderboardRevealSequence() {
  setTimeout(() => { if (state.view === 'roomResult') Sound.click(); }, 60);
  setTimeout(() => { if (state.view === 'roomResult') Sound.click(); }, 260);
  setTimeout(() => { if (state.view === 'roomResult') { Sound.victory(); celebrate(); } }, 560);
}
...
function renderPlayerProfileBody() {
  ...
  body.innerHTML = `
    ...
    ` : `<div class="chat-empty">${LOADER('inline')} Загружаем статистику...</div>`}
...
/* ============ INIT ============ */
(async function init() {
  document.getElementById('appMain').innerHTML = '<div class="empty-state">' + LOADER('full') + '</div>';
